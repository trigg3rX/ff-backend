...
export const initiateOAuth = async (
    ...
    try {
        ...
        const redirectUri = `${process.env.API_BASE_URL || 'http://localhost:3000'}/api/v1/integrations/slack/oauth/callback`;
        ...
        // Required scopes for posting messages and listing channels
        ...
        logger.error({ error, userId: req.userId }, "Failed to initiate Slack OAuth");
        throw error;
    }
};
...
export const listChannels = async (
    ...
    try {
        ...
        logger.error({ error, userId: req.userId }, "Failed to list Slack channels");
        throw error;
    }
};
    ...
    try {
        ...
        if (connection.connection_type === 'oauth') {
            ...
            if (!responseData.ok) {
                throw new AppError(
                    400,
                    `Slack API error: ${responseData.error || 'Unknown error'}`,
                    "SLACK_API_ERROR"
                );
            }
        }
